...
function SofatimeComponent(root, parent) {
  ...
  if (this.boundElements.optionList) {
    this.boundElements.optionListOptions = this.boundElements.optionList.querySelectorAll(
      "option"
    );
  }
  ...
  /*
      this.setState({
          ...
          startDatetime: '2020-01-01T17:00'
      })
      */
}
/**
...
SofatimeComponent.prototype.addEventListeners = function () {
  ...
  console.log(this.boundElements.optionList);
  if (this.boundElements.optionList) {
  ...
  for (var i = 0; i < this.boundElements.optionListOptions.length; i++) {
    ...
    try {
      var optionText =
        option.originalText +
        " (" +
        this.dayjsStartTime.tz(timezone).format(format).toUpperCase() +
        ")";
...
SofatimeComponent.prototype.renderDayjsTimes = function (day, els, is24) {
  ...
  for (let el of els) {
    let formattedString = "";
    if (is24) {
      formattedString = day.tz(this.parent.state.timezone).format(el.dataset.format.replace(/h/g, "H").replace(/a/gi,''));
    } else {
      formattedString = day.tz(this.parent.state.timezone).format(el.dataset.format.replace(/H/g, "h")+'A');
    }
...
SofatimeComponent.prototype.render = function () {
  ...
  try {
    ...
    this.renderDayjsTimes(
      this.dayjsStartTime,
      this.boundElements.startTimes,
      this.parent.state.is24
    );
    this.renderDayjsTimes(
      this.dayjsEndTime,
      this.boundElements.endTimes,
      this.parent.state.is24
    );
